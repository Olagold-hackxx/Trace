      ...
      <div className="p-6 max-w-6xl mx-auto">
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-5">
            ...
            <div className="bg-[#111111] rounded-2xl p-6" style={{ border: "1px solid #1e1e1e", boxShadow: "0px 4px 20px rgba(15,23,42,0.05)" }}>
              <h2 className="text-lg font-bold text-[#f0f0f0] mb-4" style={{ fontFamily: "Epilogue, sans-serif" }}>Similar Jobs</h2>
              <div className="space-y-3">
                {similarJobs.map((j) => (
                  <Link key={j.id} href={`${similarBasePath}/${j.id}`}
                    className="block rounded-xl p-4 hover:shadow-sm transition-all"
                    style={{ backgroundColor: "#161616", border: "1px solid #1e1e1e" }}>
                    <div className="flex items-start justify-between gap-2 mb-2">
                      <div>
                        <p className="text-sm font-bold text-[#f0f0f0]">{j.title}</p>
                        <p className="text-xs text-[#94a3b8]">{j.company}</p>
                      </div>
                      <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full"
                        style={{ backgroundColor: j.tagBg, color: j.tagColor }}>{j.tag}</span>
                    </div>
                    <p className="text-sm font-semibold" style={{ color: "#ff6b00" }}>{j.pay}</p>
                  </Link>
                ))}
              </div>
            </div>
