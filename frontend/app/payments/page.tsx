...
export default function PaymentsPage() {
  const { user, summary, transactions, paymentLinks, refresh } =
    useTraderData();
  ...
  const mappedPaymentLinks = paymentLinks.map((link) => ({
    ...
    total:
      link.amountKobo && Number(link.amountKobo) > 0
        ? formatNairaFromKobo(link.amountKobo)
        : "Flexible",
  ...
  const handleGenerateLink = async () => {
    ...
    try {
      ...
      console.log("[Trace] X-Checkout-Url header:", checkoutUrl); // 👈 check this in DevTools

      const blob = await res.blob();
      console.log("[Trace] QR blob size:", blob.size, "type:", blob.type); // 👈 and this
      ...
      console.error("[Trace] handleGenerateLink error:", error);
      setRequestError(
      ...
      <div className="p-6 max-w-6xl mx-auto">
        ...
        {showRequestForm && (
          ...
          >
            ...
            {requestError && (
              <p className="text-xs text-[#f87171] mb-3">{requestError}</p>
            )}
          ...
          <div className="grid lg:grid-cols-[1fr_260px] gap-4 mb-6">
            ...
            >
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-[#94a3b8]">
                      {user?.businessName ?? user?.fullName ?? "Trace"} ·
                      One-time · ₦{Number(requestAmount).toLocaleString()}
                    </p>
                  </div>
                </div>
                <span
              ...
              <p className="text-xs text-[#64748b]">
                Share this link or the QR code with your customer. Valid for a
                single payment of ₦{Number(requestAmount).toLocaleString()}.
              </p>
            </div>
            ...
            >
              ...
              <div
                className="rounded-xl p-3 w-full flex items-center justify-center"
              ...
              <button
                onClick={() =>
                  handleDownloadQr(requestQr, `trace-pay-${requestAmount}.png`)
                }
        ...
        {!showRequestForm && !requestLink && (
          ...
          >
            ...
            <p className="text-xs text-[#64748b]">
              Generate a QR code and checkout link to collect payment from a
              customer.
            </p>
          </div>
        )}
          ...
          <div className="space-y-4">
            {mappedPaymentLinks.length === 0 ? (
              ...
              >
                ...
                <p className="text-xs text-[#64748b]">
                  Click to generate your first payment link.
                </p>
              </div>
            ) : (
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    >
                      <LinkOutlined
                        style={{
                          fontSize: 20,
                          color: link.active ? "#ff6b00" : "#94a3b8",
                        }}
                      />
                    </div>
                    <div>
                      ...
                      <p className="text-xs text-[#94a3b8] mt-1">
                        Created {link.created} · {link.total}
                      </p>
                    </div>
                  </div>
