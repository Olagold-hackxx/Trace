  ...
  return (
    <AppShell role="lender">
      <div className="p-6 max-w-7xl mx-auto">
        {/* Header */}
        <div className="mb-6">
          <h1 className="text-2xl font-bold text-[#261812]" style={{ fontFamily: "Epilogue, sans-serif" }}>Loan Approval Queue</h1>
          <p className="text-sm text-[#8e7164] mt-1">Review and decision pending applications</p>
        </div>
        ...
        {/* Filters */}
        <div className="bg-white rounded-2xl p-4 mb-5 flex flex-wrap gap-3 items-center" style={{ border: "1px solid #e2bfb0" }}>
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-[#8e7164] mr-1">Risk:</span>
            {["All", "Low", "Medium", "High"].map((r) => (
              <button key={r} onClick={() => setRiskFilter(r)}
                className="px-3 py-1.5 rounded-lg text-xs font-semibold transition-all"
                style={riskFilter === r ? { backgroundColor: "#2563eb", color: "#fff" } : { backgroundColor: "#fff8f6", color: "#5a4136", border: "1px solid #e2bfb0" }}>
                {r}
              </button>
            ))}
          </div>
          <div className="flex items-center gap-2">
            <span className="text-sm text-[#8e7164]">Min score:</span>
            <input type="number" placeholder="e.g. 600" value={scoreMin} onChange={(e) => setScoreMin(e.target.value)}
              className="w-20 px-2 py-1.5 text-sm rounded-lg border outline-none" style={{ borderColor: "#e2bfb0", backgroundColor: "#fff8f6", color: "#261812" }} />
          </div>
        </div>
              ...
              <thead>
                <tr style={{ backgroundColor: "#fff8f6", borderBottom: "1px solid #e2bfb0" }}>
                  {["Merchant", "Business Type", "TraceScore", "Requested", "Purpose", "Risk", "Queue", "Status", "Actions"].map((h) => (
                    <th key={h} className="text-left px-4 py-3.5 font-semibold text-xs text-[#8e7164] whitespace-nowrap">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {filtered.map((app) => {
                  ...
                  return (
                    <tr key={app.id} className={`hover:bg-[#fff8f6] transition-colors ${dec ? "opacity-60" : ""}`} style={{ borderBottom: "1px solid #f8ddd2" }}>
                      <td className="px-4 py-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="font-semibold text-[#261812] text-xs">{app.name}</p>
                            <p className="text-xs text-[#8e7164]">{app.owner}</p>
                          </div>
                        </div>
                      </td>
                      <td className="px-4 py-4 text-[#5a4136] text-xs">{app.type}</td>
                      <td className="px-4 py-4">
                      ...
                      <td className="px-4 py-4 font-semibold text-[#261812] text-xs">₦{(app.amount / 1000).toFixed(0)}K</td>
                      <td className="px-4 py-4 text-[#5a4136] text-xs max-w-28 truncate">{app.purpose}</td>
                      <td className="px-4 py-4">
                      ...
                      <td className="px-4 py-4 text-xs text-[#8e7164]">{app.days}d</td>
                      <td className="px-4 py-4">
                        {dec ? (
                        ...
                        {!dec && (
                          <div className="flex items-center gap-2">
                            <Link href={`/lender/merchants/${app.id}`}>
                              <button className="p-1.5 rounded-lg border transition-all hover:bg-[#fff1eb]" style={{ borderColor: "#e2bfb0" }} title="View">
                                <Visibility style={{ fontSize: 14, color: "#5a4136" }} />
                              </button>
                            </Link>
        ...
        <p className="text-xs text-[#8e7164] mt-4 text-center">{filtered.length} applications · Showing all results</p>
      </div>
    </AppShell>
  );
}
